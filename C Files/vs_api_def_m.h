/* VS API functions in a VS DLL. Listed more or less how they appear in the VS API
  ...
  Jun 03, 10. M. Sayers. Added newer API funcions, removed ... args from some.
  Jun 06, 08. M. Sayers. Simplified from full ANSI C version.
  */
...
void     vs_initialize (double  t, 
                void (*ext_calc) (double , vs_ext_loc),
                void (*ext_echo) (vs_ext_loc));
int      vs_integrate (double  *t, 
               void (*ext_eq_in) (double , vs_ext_loc));
int      vs_integrate_io_2 (double  t, double  *imports, double  *exports, 
                                void (*ext_calc) (double , vs_ext_loc));
double   vs_setdef_and_read (const char *simfile, void (*ext_setdef) (void),
                     int (*ext_scan) (char *, char *));
int      vs_stop_run (void);
void     vs_terminate (double  t, void (*ext_echo) (vs_ext_loc));
...
int      vs_get_var_id (char *keyword, vs_sym_attr_type *type);
...
int      vs_set_sym_real (int id, vs_sym_attr_type dataType, double  value);
...
void    vs_get_road_xyz (double  s, double  l, double  *x, double  *y, 
                  double  *z);
...
void    vs_get_road_xy_j (double  s, double  l, double  *x, double  *y, int *j);
double  vs_road_curv_j (double  s, int *j);
double  vs_road_yaw_j (double  sta, double  direction, int *j);
...
int     vs_get_sensor_connections (vs_real *connect);

// configurable table functions (chapter 7)
int     vs_define_table (char *root, int ntab, int ninst);
vs_real vs_table_calc (int index, vs_real xcol, vs_real x, int itab, int inst);
int     vs_table_index (char *name);
int     vs_table_ntab (int index);
int     vs_table_ninst (int index);

void    vs_copy_table_data (vs_tab_group *tabg);
int     vs_install_keyword_tab_group (vs_tab_group *tabs);
void    vs_malloc_table_data (vs_table *tab, int type, int nx, int ny);
